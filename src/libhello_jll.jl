...
if VERSION < v"1.3.0-rc4"
    ...
    error("Unable to import libhello_jll on Julia versions older than 1.3!")
end
    ...
    function dev_jll()
        ...
        mktempdir() do temp_env
            Pkg.activate(temp_env) do
                Pkg.develop("libhello_jll")
            end
        end
        # Create the override directory
        override_dir = joinpath(Pkg.devdir(), "libhello_jll", "override")
        # Copy the current artifact contents into that directory
        if !isdir(override_dir)
            cp(artifact"libhello", override_dir)
        end
...
artifacts = Pkg.Artifacts.load_artifacts_toml(artifacts_toml; pkg_uuid=UUID("3939d790-347f-5599-b9e1-f2daa7662b6b"))
platforms = [Pkg.Artifacts.unpack_platform(e, "libhello", artifacts_toml) for e in artifacts["libhello"]]
...
if best_platform === nothing
    @debug("Unable to load libhello; unsupported platform $(triplet(platform_key_abi()))")
else
